CREATE FUNCTION public.reportes_peluqueros_obtener()
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
CREATE FUNCTION public.reportes_sucursales_obtener()
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
            ...
            JSONB_BUILD_OBJECT(
                ...
                'completadas', (
                    ...
                    WHERE c.sucursal_id = s.id
                      AND c.activo = TRUE
                      AND (c.estado = 'atendida' OR rs.estado = 'completado')
                ),
                'proximas', (
                    SELECT COUNT(*)
                    FROM public.citas c
                    WHERE c.sucursal_id = s.id
                      ...
                      AND c.estado NOT IN ('atendida', 'cancelada', 'no_asistio')
                )
            ) ORDER BY s.nombre
        ), '[]'::JSONB), 'total', COUNT(*)
    ) INTO v_resultado
    ...
    WHERE s.activo = TRUE
      AND petstore_private.tiene_acceso_sucursal(s.id);
