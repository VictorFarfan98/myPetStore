ALTER TABLE public.paquetes
    ADD COLUMN vigencia_dias INTEGER NOT NULL DEFAULT 90
...
SET fecha_expiracion = ((pa.asignado_en AT TIME ZONE 'America/Guatemala')::DATE + p.vigencia_dias)
FROM public.paquetes p
WHERE p.id = pa.paquete_id;
...
DROP FUNCTION public.paquetes_crear(TEXT, NUMERIC, JSONB);
...
BEGIN
    ...
    INSERT INTO public.paquetes (nombre, precio, vigencia_dias, creado_por_usuario_id)
    VALUES (BTRIM(p_nombre), p_precio, p_vigencia_dias, v_actor)
    ...
    PERFORM petstore_private.auditar_cambio(
        'paquetes', v_paquete.id::TEXT, 'insertar', NULL,
        TO_JSONB(v_paquete) || JSONB_BUILD_OBJECT('servicios', p_servicios), NULL, NULL
    );
    RETURN v_paquete;
EXCEPTION
...
BEGIN
    ...
    INSERT INTO public.paquetes_asignaciones (
        paquete_id, cliente_id, precio_pagado, fecha_expiracion, asignado_por_usuario_id
    )
    VALUES (v_paquete.id, v_cliente.id, v_paquete.precio, v_fecha_expiracion, v_actor)
...
REVOKE ALL ON FUNCTION public.paquetes_crear(TEXT, NUMERIC, INTEGER, JSONB) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.paquetes_crear(TEXT, NUMERIC, INTEGER, JSONB) TO authenticated, service_role;
