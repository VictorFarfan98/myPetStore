...
function SheetForm({ data, appointmentId, record, canDelete, onClose, onSaved, onRefresh }: { data: AppData; appointmentId: number; record?: GroomingRecord; canDelete: boolean; onClose: () => void; onSaved: (result: { completed?: boolean; recordId?: number; groomerId?: number }) => void; onRefresh: () => void }) {
...
export function HojasBrowser({
  ...
  const canManagePayments = ["administrador", "propietario", "encargado"].includes(data.currentUserRole ?? "");
  ...
  return <div>
    ...
    <div className="space-y-4">{appointments.map((appointment) => {
      ...
      const canRegisterPayment = canManagePayments && !cancelled && readyForPayment;
      const canCancel = !isWorker && !["cancelled", "completed", "in_progress", "checked_in", "no_show"].includes(appointment.status);
