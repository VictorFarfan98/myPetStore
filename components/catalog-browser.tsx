...
export function CatalogBrowser({
  ...
  const [form, setForm] = useState({ id: "", nombre: "", especie: "perro" as "perro" | "gato" | "otro", activo: true, prices: {} as Record<string, string> });
  ...
  function reset() {
    setEditing(false);
    setForm({ id: "", nombre: "", especie: "perro", activo: true, prices: {} });
  }
  ...
  return <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
    <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-panel">
      ...
      <DataTable rows={rows} columns={[
        ...
        { key: "acciones", header: "Acciones", render: (row) => <div className="flex gap-3"><button className="font-semibold text-jade hover:underline" onClick={() => { setEditing(true); setForm({ id: String(row.id), nombre: row.nombre, especie: row.especie ?? "perro", activo: row.activo, prices: Object.fromEntries(priceRows.filter((price) => price.shampoo_id === row.id).map((price) => [price.tamano_id, price.recargo])) }); }} type="button">Editar</button><button className="font-semibold text-red-700 hover:underline" onClick={() => deleteRow(row.id)} type="button">Eliminar</button></div> }
      ]} />
    </div>
