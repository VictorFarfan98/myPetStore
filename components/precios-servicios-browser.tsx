...
type Form = { servicio_id: string; tamano_id: string; precio: string; duracion_minutos: string; activo: boolean };
const emptyForm: Form = { servicio_id: "", tamano_id: "", precio: "", duracion_minutos: "", activo: true };
...
export function PreciosServiciosBrowser({ rows, services, sizes }: Props) {
  ...
  return <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
    <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-panel">
      <div className="mb-5"><h2 className="text-xl font-semibold text-ink">Precios y duración por tamaño</h2><p className="mt-1 text-sm text-slate-500">Configura el precio y tiempo estimado de cada servicio.</p></div>
      <DataTable rows={rows.map((row) => ({ ...row, id: `${row.servicio_id}-${row.tamano_id}` }))} columns={[
        { key: "servicio", header: "Servicio", render: (row) => <span className="font-semibold text-ink">{serviceName(row.servicio_id)}</span> },
        { key: "tamano", header: "Tamaño", render: (row) => sizeName(row.tamano_id) },
    ...
    <form className="h-fit rounded-lg border border-slate-200 bg-white p-5 shadow-panel" onSubmit={submit}>
      ...
      <div className="mt-5 space-y-4">
        ...
        <label className="block text-sm font-medium text-ink">Tamaño<select className="focus-ring mt-1 w-full rounded-lg border border-slate-300 px-3 py-2.5 font-normal disabled:bg-slate-100" disabled={editing} name="tamano_id" onChange={(event) => setForm({ ...form, tamano_id: event.target.value })} required value={form.tamano_id}><option value="">Selecciona un tamaño</option>{sizes.map((size) => <option key={size.id} value={size.id}>{size.nombre}{size.activo ? "" : " (inactivo)"}</option>)}</select>{editing && <input name="tamano_id" type="hidden" value={form.tamano_id} />}</label>
