...
export function ClientesBrowser({ data, page, initialQuery, canManageFidelity }: { data: ClientesData; page: number; initialQuery: string; canManageFidelity: boolean }) {
  ...
  const [adjustingFidelity, setAdjustingFidelity] = useState<{ customerId: number; serviceId: number } | null>(null);
    ...
    <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
      <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-panel">
        ...
        <div className="mt-5"><DataTable rows={rows.map(({ customer, pets }) => ({ ...customer, pets }))} columns={[
          ...
          { key: "fidelidad", header: "Fidelidad", render: (row) => <div className="w-48 space-y-3">{row.loyaltyProgress?.length ? row.loyaltyProgress.map((progress) => { const percentage = Math.round(progress.completed / progress.required * 100); return <div key={progress.serviceId} aria-label={`${progress.serviceName}: ${progress.completed} de ${progress.required} servicios completados`}><div className="mb-1 flex justify-between gap-2 text-xs"><span className="truncate font-medium" title={progress.serviceName}>{progress.serviceName}</span><span>{progress.completed}/{progress.required}</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-jade" style={{ width: `${percentage}%` }} /></div>{canManageFidelity && <button className="mt-1 text-xs font-semibold text-amber-700 hover:underline" type="button" onClick={() => setAdjustingFidelity({ customerId: row.id, serviceId: progress.serviceId })}>Ajustar</button>}</div>; }) : <span>—</span>}</div> },
          { key: "acciones", header: "Acciones", render: (row) => { const selectedProgress = row.loyaltyProgress?.find((progress) => progress.serviceId === adjustingFidelity?.serviceId); return adjustingFidelity?.customerId === row.id && selectedProgress ? <form className="grid min-w-48 gap-2" onSubmit={submitFidelityAdjustment}><input name="cliente_id" type="hidden" value={row.id} /><input name="servicio_id" type="hidden" value={selectedProgress.serviceId} /><p className="text-xs font-semibold text-slate-600">{selectedProgress.serviceName}</p><label className="text-xs font-medium text-slate-600">Servicios<input className="focus-ring mt-1 w-full rounded border border-slate-300 px-2 py-1" defaultValue={selectedProgress.completed} min="0" name="completados" required type="number" /></label><label className="text-xs font-medium text-slate-600">Motivo<input className="focus-ring mt-1 w-full rounded border border-slate-300 px-2 py-1" name="motivo" placeholder="Depuración" required /></label><div className="flex gap-2"><button className="font-semibold text-jade hover:underline" disabled={isPending} type="submit">Guardar</button><button className="font-semibold text-slate-500 hover:underline" onClick={() => setAdjustingFidelity(null)} type="button">Cancelar</button></div></form> : <div className="flex gap-3"><button className="font-semibold text-jade hover:underline" type="button" onClick={() => edit(row)}>Editar</button><button className="font-semibold text-red-700 hover:underline" type="button" onClick={() => remove(row.id)}>Eliminar</button></div>; } }
        ]} /></div>
