...
const emptyForm = { id: "", nombre: "", intervalo_recordatorio_dias: "", precio: "", activo: true };
    ...
    <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
      <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-panel">
        ...
        <DataTable rows={visibleRows} columns={[
          ...
          ...(additional ? [{ key: "precio", header: "Precio", render: (row: ServicioRow) => row.precio ? `Q ${Number(row.precio).toFixed(2)}` : "—" }] : []),
      ...
      <form className="h-fit rounded-lg border border-slate-200 bg-white p-5 shadow-panel" onSubmit={submit}>
        ...
        <div className="mt-5 space-y-4">
          ...
          {additional && <label className="block text-sm font-medium text-ink">Precio (GTQ)<input className="focus-ring mt-1 w-full rounded-lg border border-slate-300 px-3 py-2.5 font-normal" min="0.01" name="precio" onChange={(event) => setForm({ ...form, precio: event.target.value })} required step="0.01" type="number" value={form.precio} /></label>}
