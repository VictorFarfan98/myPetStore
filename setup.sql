...
DROP FUNCTION IF EXISTS public.calificaciones_groomer_insertar(BIGINT, SMALLINT, TEXT) CASCADE;
...
CREATE UNIQUE INDEX uq_tamanos_nombre_lower
    ON public.tamanos (LOWER(nombre));
...
CREATE TABLE public.mascotas (
    ...
    nombre TEXT NOT NULL
        CHECK (BTRIM(nombre) <> ''),
    especie public.especie_mascota NULL,
    raza TEXT NULL,
...
CREATE TABLE public.opciones_shampoo (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    nombre TEXT NOT NULL
        CHECK (BTRIM(nombre) <> ''),
    activo BOOLEAN NOT NULL DEFAULT TRUE,
...
CREATE UNIQUE INDEX uq_opciones_shampoo_nombre_lower
    ON public.opciones_shampoo (LOWER(nombre));

CREATE TABLE public.precios_shampoo (
    shampoo_id BIGINT NOT NULL
        REFERENCES public.opciones_shampoo(id) ON DELETE RESTRICT,
    tamano_id BIGINT NOT NULL
        REFERENCES public.tamanos(id) ON DELETE RESTRICT,
...
CREATE TABLE public.registros_servicio (
    ...
    recargo_shampoo NUMERIC(10, 2) NOT NULL DEFAULT 0.00
        CHECK (recargo_shampoo >= 0),
    ...
    CHECK (
        ...
        OR monto_final = precio_base + recargo_shampoo - descuento_cupon
    ),
    CHECK (
    ...
    CHECK (
        estado <> 'completado'
        OR servicio_id NOT IN (1, 3)
        OR shampoo_id IS NOT NULL
    )
);
...
CREATE TRIGGER trg_opciones_shampoo_actualizado_en
BEFORE UPDATE ON public.opciones_shampoo
FOR EACH ROW EXECUTE FUNCTION public.establecer_actualizado_en();

CREATE TRIGGER trg_precios_shampoo_actualizado_en
BEFORE UPDATE ON public.precios_shampoo
FOR EACH ROW EXECUTE FUNCTION public.establecer_actualizado_en();
...
BEGIN
    ...
    IF v_transicion_a_completado THEN
        ...
        IF NEW.servicio_id IN (1, 3) AND NEW.shampoo_id IS NULL THEN
            RAISE EXCEPTION 'El shampoo es obligatorio para Baño y Grooming';
        END IF;
...
INSERT INTO public.servicios (id, nombre, activo)
VALUES
    (1, 'Baño', FALSE),
    (2, 'Corte de uñas', FALSE),
    (3, 'Grooming', FALSE);

INSERT INTO public.opciones_shampoo (id, nombre, activo)
VALUES
    (1, 'Estándar', FALSE),
    (2, 'Antipulgas', FALSE),
    (3, 'Medicado', FALSE);
...
SELECT setval(
    pg_get_serial_sequence('public.opciones_shampoo', 'id'),
    COALESCE((SELECT MAX(id) FROM public.opciones_shampoo), 1),
    EXISTS (SELECT 1 FROM public.opciones_shampoo)
);
